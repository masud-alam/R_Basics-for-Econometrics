...
# example 2.3, page 32
#run the regression like old fashioned way, 
#but it gives precise understanding what we are doing
...
# What does OLS perform?

cov(roe,salary)
var(roe)
mean(salary)
mean(roe)
beta1hat <- cov(roe,salary)/var(roe)
beta1hat
beta0hat <- mean(salary)-beta1hat*mean(roe)
beta0hat
#Now run the regression smarter way

ceoreg <- lm(salary~roe) 
summary(ceoreg)
